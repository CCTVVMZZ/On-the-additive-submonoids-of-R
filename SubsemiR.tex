...
\begin{proof}
  ...
  % Let $Y$ denote the set of those real numbers that can be written
  % as the sum of at most $m$ elements of $X \cap [0, b]$.
  % By construction, $Y$ is finite:
  % the cardinality of $Y$ is not greater than that 
  % $\left( n^{m + 1} - 1 \right) {(n -  1)}^{-1}$,
  % where $n$ 
  % for .
 % By construction, $Y$ is finite:
 %  each element of $Y$ can be written as the sum of exactly $m$ elements of $\left(X \cap [0, b] \right) \cup \{ 0 \}$,
 % so the cardinality of $Y$ is not greater than that of $\left(X \cap [0, b] \right) \cup \{ 0 \}$ raised to the power~$m$.
  %${(n + 1)}^m$,
  % The cardinality of $Y$ is not greater than ${(n + 1)}^m$,
  % where $n$ denotes the cardinality of $X \cap [0, b]$.
  %The final step is to check $M \cap [0, b] \subseteq Y$.
  ...
  $$
  %Therefore, $v$ is an element of~$Y$.
\end{proof}
