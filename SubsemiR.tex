...
% \begin{lemma}
%   Let $a$, $h \in \Rnneg$.
%   The additive closure of $\left]a, a + h \right[$ is equal to $\left] a, \infty \right[$
%   if, and only if, $h$ is larger than~$a$.
% \end{lemma}

% \begin{proof}
%   Put $I = \left]a, a + h \right[$: $n I  = \left] n a , n a + n h \right[$ for every $n \in \Rpos$.
%   First, assume that the additive closure of $I$ is equal to $\left]a, \infty \right[$.
%   Assume
...
\begin{proof}
  ...
  % and
  % $$
  % U' = U \cap  \left[1, \infty \right[ \, .  
  % $$
  % Since $1 \in U$,
  % our task is to prove that both $U'$ and $U \cap \left]0, 1 \right]$ are non-trivial intervals.
  % Since
  % $$
  % U \cap \left]0, 1 \right] =  \left\{ u^{-1} : u \in U' \right\} \,,
  % $$
  % $U'$ is an interval if, and only if, $U \cap \left]0, 1\right]$ is an interval.
  First, assume $0 \in I$.
...
\begin{proof}
 ...
 % we may assume $I = [a, b]$.
  ...
  $$
  %It follows that the additive closure of $I$ contains $[m a, n b]$ as subset for every $n \in \
  
 %  and
 %  $k \ge {(b - a)}^{-1} b$ is equivalent to $(k + 1) b \le k a$.
 %  For every $k \in \bR$, %the following three assertions are equivalent:
 %  $k \ge {(b - a)}^{-1} a$ is equivalent to $(k + 1) a  \le k b$
 %  and
 %  $k \ge {(b - a)}^{-1} b$ is equivalent to $(k + 1) b \le k a$.
 % % , and $(k + 1)a \in [k a, k b]$.
  
  
  % such that $I \subseteq M$.
  % Put
  % $$
